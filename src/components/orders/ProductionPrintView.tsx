...
const getCategory = (productName: string): string => {
  ...
  if (lowerName.includes("sem glúten") || lowerName.includes("gluten free")) {
    return "SEM GLÚTEN";
  }
...
export function ProductionPrintView({
  orders,
  dateFilter,
}: ProductionPrintViewProps) {
  // Aggregate data
      ...
      <div className="grid grid-cols-2 gap-8">
              ...
              <div key={cat} className="border-2 border-[#000000]">
                <table className="w-full border-collapse">
                  <thead>
                    ...
                    >
                      <th className="text-left p-1 pl-2 border-r-2 border-[#000000] font-bold text-sm">
                        {cat}
                      </th>
                      {storeColumns.map((store) => (
                        <th
                          key={store}
                          className="w-8 text-center border-r-2 border-[#000000] last:border-r-0 font-bold text-sm"
                        >
                          {store}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {products.map((prod, idx) => {
                      ...
                      return (
                        ...
                        >
                          <td className="p-1 pl-2 border-r-2 border-[#000000] text-xs font-medium truncate max-w-[200px]">
                            {prod}
                          </td>
                          {storeColumns.map((store) => (
                            <td
                              key={store}
                              className="text-center border-r-2 border-[#000000] last:border-r-0 text-xs font-bold"
                            >
                              {storeMap.get(store) || ""}
                            </td>
                          ))}
        ...
        <div className="space-y-6">
          ...
          <div className="border-2 border-[#000000]">
            ...
            >
              <div className="border-r-2 border-[#000000] p-1">
                INSUMOS EXTRAS
              </div>
              <div className="p-1">LOJA</div>
            </div>
